...
export function GameActions({ state, dispatch }: GameActionsProps) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-30 border-t border-amber-200/20 bg-[#071d19]/95 px-4 py-3 backdrop-blur md:static md:rounded-md md:border">
      <div className="mx-auto flex w-full max-w-full min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="min-w-0 text-sm text-emerald-50">
          선택 카드 {selectedCards.length}장: <span className={validation.valid ? "text-green-300" : "text-amber-200"}>{validation.reason}</span>
        </p>
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="rounded bg-amber-300 px-4 py-2 text-sm font-semibold text-emerald-950 disabled:cursor-not-allowed disabled:opacity-40"
          >
            카드 제출
          </button>
          <button
          ...
          <button
            ...
            className="rounded border border-white/20 px-4 py-2 text-sm text-emerald-50"
          >
            선택 취소
          </button>
        </div>
